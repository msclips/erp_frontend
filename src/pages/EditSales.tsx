...
export default function EditSales() {
  ...
  useEffect(() => {
    if (orderData) {
      setFormData({
        customer: getCustomerValue(orderData.customer),
  ...
  // Helper function to map customer names to values
  const getCustomerValue = (customerName: string) => {
    const customerMap: { [key: string]: string } = {
      "Acme Corporation": "acme",
      "TechStart Ltd": "techstart",
      "Global Dynamics": "global",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Updating sales order:", { ...formData, id: orderData?.id });
    
    toast({
      title: "Sales Order Updated",
      description: `Sales order ${orderData?.id} has been updated successfully`,
    });
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-center text-muted-foreground">
                Please select an order from the sales page to edit.
              </p>
              <div className="flex justify-center mt-4">
                <Button onClick={handleBack}>Go Back to Sales</Button>
              </div>
            </CardContent>
          </Card>
        </div>
    ...
    <Layout currentPath="/sales">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Edit Sales Order</h1>
